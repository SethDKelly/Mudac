...
This file is a **bootstrap adapter**, not the canonical source of MUDAC product, synchronization, architecture, implementation, verification, source-topology, runtime, persistence, or documentation rules.
...
11. For persistence, temporal/history representation, Version/Provenance storage, governed-exception storage, outbox, projection or migration work, additionally load [`Persistence, History, Provenance, Outbox, Projection & Migration Implementation Contract`](docs/canonical/implementation/persistence-history-projection.md) plus only the task-relevant upstream semantic/architecture owners. [008-D](docs/008-implementation-reentry/008-D-persistence-temporal-truth-versioning-provenance-governed-exceptions-outbox-projection-migration-implementation-plan.md) is rationale/planning provenance; the canonical implementation contract owns current physical choices.
12. Verification/test work additionally loads [`Verification Strategy, Evidence & Quality Gates`](docs/canonical/implementation/verification-strategy.md).
13. Source/package/import work additionally loads [`Source Topology, Package Boundaries & Dependency Enforcement`](docs/canonical/implementation/source-topology.md).
14. Runtime/environment/CI/IaC work additionally loads [`Runtime, Environment & Delivery Bootstrap`](docs/canonical/implementation/runtime-delivery-bootstrap.md).
15. Use numbered phase history only for rationale, chronology, rejected alternatives, implementation lineage, or source audit.
...
Phase 008 is active. 008-A established implementation-planning authority; 008-B qualified the retained 006-D substrate; 008-C reconciled accepted residual/historical work; 008-D accepted the persistence/history implementation contract.
...
The executable work created through 006-D is retained as a **qualified protected non-domain implementation baseline**. 008-D defines what future persistence implementation must build but does not alter the schema-free executable baseline.
...
Current work should proceed to **008-E — Identity, Authentication, Participation, Access, Session, Invitation, Secrets & Technical-Authority Implementation Plan**, not domain coding.
...
- stable UUID resource identity and per-root monotonic `bigint` concurrency revisions;
- mutable root revision remains distinct from semantic Version identity;
- committed Versions and meaningful Provenance/correction history are append-stable;
- no universal bitemporal/status/soft-delete model collapses temporal dimensions;
- governed exceptions are policy-specific immutable decisions, not a universal override table;
- Official Outcome Revision uses immutable outcome-owned revisions/basis plus a separate latest-declared pointer;
- transactional outbox delivery is at-least-once and queue order is not authoritative commit order;
- projections preserve explicit source basis and converge idempotently under duplicate/out-of-order messages; non-trivial rebuilds use generation swap by default;
- migrations are SQL-first, owner-scoped, forward in production posture, checksum-verified, advisory-lock protected, separately privileged and never application-startup auto-migrations;
- authoritative historical evidence is retained conservatively until 008-K establishes applicable retention requirements;
- no new generic persistence/infrastructure workspace package is justified.

These choices are implementation contracts, not permission to create their executable artifacts.
...
Key routing consequences remain:
...
- create the 008-D planned schemas, migrations, outbox, projections or repositories before 008-L authorization;
...
- use generic `status`, `deleted_at`, JSON payloads or cascade deletion to collapse/erase lifecycle, currentness, validity, replacement, correction, Affected/Stale or historical distinctions;
...
- infer that completion of Concept Design, 008-A/B/C/D, a qualified bootstrap, accepted persistence plan, green checks, historical 005-J readiness, or old 006-E–M plans authorize skipping the 008-L first-slice boundary;
...
The selected implementation family remains Node.js 24 LTS + TypeScript, pnpm workspaces, Fastify, Kysely + node-postgres, explicit SQL-first migrations, outward-generated OpenAPI, Vitest/Playwright, strict TypeScript + ESLint + Prettier, and OpenTofu.
