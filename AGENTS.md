...
This file is a **bootstrap adapter**, not the canonical source of MUDAC product, architecture, implementation, verification, or documentation rules.
...
6. Read only the category/owner documents materially relevant to the task.
7. Follow stable rule IDs and linked dependencies as needed.
8. Use numbered phase history only for rationale, chronology, rejected alternatives, implementation planning lineage, or source audit.
...
If implementation, test behavior, or architecture conflicts with canonical meaning and no redesign was requested, the downstream mechanism must adapt.
...
Exact package/source/test topology is not implied by this bootstrap or the knowledge tree; Phase 006-C owns that boundary. Exact package versions are pinned in implementation manifests/lockfiles when introduced and are not permanent canonical semantics.
...
As implementation tooling is introduced, run the applicable type/lint/test/security/generated-code checks defined by `IMPL-*` and the current verification owner.
