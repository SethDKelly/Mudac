...
Current accepted architecture is routed through [canonical/architecture/](canonical/architecture/): the [Architectural Foundation](canonical/architecture/architectural-foundation.md) owns `ARCH-*`, [Application Boundaries](canonical/architecture/application-boundaries.md) owns `MOD-*`, [Data & Persistence Architecture](canonical/architecture/data-persistence.md) owns `DATA-*`, [Identity, Authentication, Access & Session Architecture](canonical/architecture/identity-access-session.md) owns `AUTH-*`, and [Commands, Queries, API, Transaction & Concurrency Architecture](canonical/architecture/commands-api-concurrency.md) owns `API-*`.

The current request/authority posture uses a versioned HTTPS/JSON browser API with explicit command/query separation. Authoritative commands re-evaluate current Access/preconditions, confirm success only after commit, use optimistic concurrency by default with targeted locking/isolation when required, and rely on durable idempotency plus domain uniqueness for safe retry. Projection-backed reads expose freshness/basis where material and never become command authority. Concrete framework/router, OpenAPI tooling, queue/broker, offline synchronization, and AWS API/runtime products remain open.
