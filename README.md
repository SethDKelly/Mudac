...
[AWS Runtime, Security & Operations Architecture](docs/canonical/architecture/aws-runtime-operations.md) defines `AWS-*` rules. Production is single-active `us-east-2`, Multi-AZ, and CloudFront-fronted with private S3 and VPC-origin/internal-ALB origins. The modular monolith runs on ECS/Fargate; RDS PostgreSQL Multi-AZ is authoritative persistence; SQS carries retryable async work; private evidence/Artifacts live in encrypted/versioned S3; deployment uses GitHub Actions OIDC rather than static AWS keys; CloudWatch/ADOT/Application Signals provide infrastructure and semantic telemetry; RDS/S3 recovery data is protected across Regions; and whole-Region live-event failure falls back to paper until a single restored digital authority is explicitly promoted.

Concrete backend implementation language/framework, package manager/build/CSS/component-library choices, IndexedDB wrapper/service-worker implementation, OpenAPI tooling, ORM/unit-of-work implementation, exact session/idempotency stores and timings, IaC tool, exact ECS/RDS sizes, queue worker library, signed-delivery implementation, and measured numeric SLO/RTO/RPO targets remain implementation decisions or measured outputs. The intended deployment boundary is now concretely **GitHub Actions → AWS via OIDC-federated roles**.
